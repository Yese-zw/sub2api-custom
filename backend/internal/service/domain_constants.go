...
const (
	...
	// 邮件服务设置
	SettingKeyEmailProvider           = "email_provider" // 邮件发送服务：smtp/resend/cloudflare
	...
	SettingKeyResendAPIKey            = "resend_api_key" // Resend API Key（加密存储）
	SettingKeyCloudflareAccountID     = "cloudflare_email_account_id"
	SettingKeyCloudflareEmailAPIToken = "cloudflare_email_api_token" // Cloudflare Email Sending API Token（加密存储）
	...
	SettingKeyCommunityGroupImage         = "community_group_image"         // 交流群图片 (base64)
	SettingKeyUITheme                     = "ui_theme"                      // 界面主题（pixel/original）
